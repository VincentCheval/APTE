...
let rec display_context = function
  ...
  | CFunc(f,args) when Term.is_tuple f ->
      Printf.sprintf "(%s)" (display_list_recipe display_context args) 
...
type 'a general_formula =
  {
    ...
    map_exists_to_free : (variable * variable) list;
    map_free_to_exists : (variable * variable) list
  }
...
let display_atomic_formula = function
  | Neq(_,[],[]) -> Debug.internal_error "[recipe.ml > display_atomic_formula] This case should not happen"
...
let display_partition_elt = function
  | PartEmpty(g_form_l) -> Printf.printf "********Partition EMPTY:\n";
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) g_form_l
  | PartVar(var,g_form_l) -> Printf.printf "********Partition VAR with var = %s\n" (display_variable var);
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) g_form_l
  | PartPath(path,g_form_l) -> Printf.printf "********Partition PATH with path = %s\n" (display_path path);
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) g_form_l
  | PartSymb(symb,var_l,g_form_l) -> Printf.printf "********Partition SYMB with symb = %s and var list = " (Term.display_symbol_with_arity symb);
      List.iter (fun var -> Printf.printf "%s, " (display_variable var)) var_l;
      print_string "\n";
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) g_form_l
  | PartNeq(f_list,path_l,g_form_l) -> Printf.printf "********Partition NEQ with symb_l = ";
      List.iter (fun f -> Printf.printf "%s, " (Term.display_symbol_with_arity f)) f_list;
      print_string " and path = ";
      List.iter (fun path -> Printf.printf "%s, " (display_path path)) path_l;
      print_string "\n";
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) g_form_l
...
let rec add_formula_in_partition_from_eq current_g_form target_var context atm_list prev_partition current_partition = match current_partition,context with
  | [], CVar(var) ->
      if var.free
      then (PartVar (var,[{current_g_form with formula = atm_list}]))::prev_partition
      else 
        begin
          ...
          let general_formula = 
            { current_g_form with
              ...
              map_free_to_exists = (target_var,var)::current_g_form.map_free_to_exists
            }
          in
          var.link <- NoLink;
          generate_new_partition target_var general_formula prev_partition [] general_formula.formula 
        end
        
  | [],CPath(path) -> (PartPath (path, [{ current_g_form with formula = atm_list}]))::prev_partition
  ...
  | [],CFunc(symb,c_list) -> 
      ...
      let general_formula = {current_g_form with formula = new_atm_list} in
      
      (PartSymb (symb,list_var,[general_formula]))::prev_partition
      
  | PartVar(var,l)::q,CVar(var') when is_equal_variable var var' -> 
      let general_formula = { current_g_form with formula = atm_list} in
      ((PartVar (var,general_formula::l))::q)@prev_partition
      
  | PartEmpty(_)::_,CVar(var) when not var.free -> 
      var.link <- VLink target_var;
      let general_formula = 
        { current_g_form with
          formula = List.map (function 
      ...
      in
      var.link <- NoLink;
      generate_new_partition target_var general_formula (prev_partition@current_partition) [] general_formula.formula
      
  | PartPath(path,l)::q, CPath(path') when is_equal_path path path' -> 
      let general_formula = { current_g_form with formula = atm_list} in
      ((PartPath(path,general_formula::l))::q)@prev_partition
  ...
  | PartSymb(symb,v_list,l)::q, CFunc(symb',c_list) when Term.is_equal_symbol symb symb' ->
      ...
      let general_formula = { current_g_form with formula = new_atm_list} in
      ((PartSymb (symb,v_list,general_formula::l))::q)@prev_partition
      
  | t::q, _ -> add_formula_in_partition_from_eq current_g_form target_var context atm_list (t::prev_partition) q
  
  
and add_formula_in_partition_from_neq current_g_form atm_list (symb_l,path_l) = function
  | [] -> 
      let general_formula = { current_g_form with formula = atm_list} in
  ...
  | t::q -> t::(add_formula_in_partition_from_neq current_g_form atm_list (symb_l,path_l) q)
  
and add_formula_in_partition_from_empty general_formula = function 
...
and generate_new_partition target_var general_formula current_partition atm_list = function
  ...
  | Eq(var,context)::q' when is_equal_variable var target_var -> 
      add_formula_in_partition_from_eq 
        ...
        []
        current_partition
  | t::q' -> generate_new_partition target_var general_formula current_partition (t::atm_list) q'

let rec partitionate target_var = function
  | [] -> []
  | general_formula::q ->
      let current_partition = partitionate target_var q in  
      generate_new_partition target_var general_formula current_partition [] general_formula.formula
...
(* return true if all solution of formula2 are also solutions of formula1 *)
let rec is_subsume_formula formula1 formula2 = match formula1,formula2 with
  | [],_ -> true
  | _,[] -> false
  | Eq(v1,c1)::q1, Eq(v2,c2)::q2 when is_equal_variable v1 v2 && is_equal_context c1 c2 -> is_subsume_formula q1 q2
  | Neq(v1,f_list1,p_list1)::q1,Neq(v2,f_list2,p_list2)::q2 when 
    is_equal_variable v1 v2 && 
    is_equal_list Term.is_equal_symbol f_list1 f_list2 &&
    is_equal_list is_equal_path p_list1 p_list2 ->
      is_subsume_formula q1 q2
  | atm1::_,atm2::_ when order_atomic atm1 atm2 = -1 -> false
  | _,_::q -> is_subsume_formula formula1 q
  
let is_equal_general_formula g_formula1 g_formula2 = 
  if !linked_variables_var <> []
  then Debug.internal_error "[recipe.ml >> is_equal_general_formula] The linked variables list should be empty"
  else 
    begin
      let result = is_equal_formula g_formula1.formula g_formula2.formula in
      cleanup_var ();
      result
    end
  
let is_subsume_general_formula g_formula1 g_formula2 = 
  if !linked_variables_var <> []
  then Debug.internal_error "[recipe.ml >> is_subsume_general_formula] The linked variables list should be empty"
  else 
    begin
      let result = is_subsume_formula g_formula1.formula g_formula2.formula in
      cleanup_var ();
      result
    end
...
let search_in_maps var g_form =
  try 
    let (_,var') = List.find (fun (v_f,_) -> is_equal_variable v_f var) g_form.map_free_to_exists in
    let (_,var'') = List.find (fun (v_e,_) -> is_equal_variable v_e var') g_form.map_exists_to_free in
    var''
  with Not_found -> var
  ...
  then []
    ...
    try
      ...
      List.iter (fun g_form_1 -> 
        partvar := List.map (fun (v,g_form_l_2) -> (v,List.filter (fun g_form_2 -> not (is_subsume_general_formula g_form_1 g_form_2)) g_form_l_2)) !partvar; 
        partpath := List.map (fun (p,g_form_l_2) -> (p,List.filter (fun g_form_2 -> not (is_subsume_general_formula g_form_1 g_form_2)) g_form_l_2)) !partpath; 
        partsymb := List.map (fun (f,v_list,g_form_l_2) -> (f,v_list,List.filter (fun g_form_2 -> not (is_subsume_general_formula g_form_1 g_form_2)) g_form_l_2)) !partsymb; 
        partneq := List.map (fun (f_list,p_list,g_form_l_2) -> (f_list,p_list,List.filter (fun g_form_2 -> not (is_subsume_general_formula g_form_1 g_form_2)) g_form_l_2)) !partneq
        ) !partempty;
      ...
      let rec search_matching_symbol f_list g_form partition_symb = match f_list, partition_symb with
        ...
        | symb::q_symb, (symb',v_list,g_form_l)::q_part when Term.is_equal_symbol symb symb' ->
            ...
            begin match l_equal with
              | [] -> 
                  if List.exists (fun g_form_sub -> is_subsume_general_formula g_form_sub g_form) g_form_l
                  then 
                    begin match search_matching_symbol q_symb g_form q_part with
                      | None -> None
                      | Some(new_partition) -> Some((symb,v_list,g_form_l)::new_partition)
                    end
                  else None
              | [_] -> 
                  begin match search_matching_symbol q_symb g_form q_part with
              ...
              | [] ->
                  ...
                  then 
                    begin match search_matching_path q_path g_form q_part with
                      | None -> None
                      | Some(new_partition) -> Some((path,g_form_l)::new_partition)
                    end
                  else None
              | [_] -> 
                  begin match search_matching_path q_path g_form q_part with
            ...
            begin match search_matching_symbol f_list g_form !partsymb with
              ...
              | Some(new_partsymb) ->
                  begin match search_matching_path p_list g_form !partpath with
                    | None -> g_form::(search_sub_neq f_list p_list g_form_l)
                    | Some(new_partpath) ->
                        let new_g_form = { g_form with neg_to_remove = (search_in_maps target_var g_form)::g_form.neg_to_remove } in
      ...
      (* DEBUG *)
      (*if List.length general_form_list > 1
      then begin
      Printf.printf "----------------------------\n";
      Printf.printf "----------------------------\n";
      Printf.printf "-- Before factorisation :\n";
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) general_form_list;
  
      
      Printf.printf "\n-- Begin PARTITION with target var = %s -----------\n" (display_variable target_var);
      
      List.iter display_partition_elt partition;
      
      Printf.printf "\n-- Begin PARTITION Factorised with target var = %s -----------\n" (display_variable target_var);
      
      List.iter display_partition_elt partition_factorised;
      end;*)
      (* END DEBUG *)
      
      let result = list_general_formula_5 in
      
      
      (* DEBUG *)
      (*if List.length general_form_list > 1
      then begin
      Printf.printf "After factorisation :\n";
      List.iter (fun g_form -> Printf.printf "%s\n" (display_general_formula g_form)) result;
      Printf.printf "End of factorisation \n\n"
      end;*)
      (* END DEBUG *)
...
let rec existentialise_context free_vars map = function
  | CVar({ link = VLink v;_}) -> CVar(v) 
  ...
  | CVar(var) -> 
      ...
      map := (new_var,var) :: !map;
      CVar(new_var)
  | CFunc(f,c_list) -> CFunc(f,List.map (existentialise_context free_vars map) c_list)
  | c -> c
      
let existentialise_atomic_formula free_vars map = function
  | Neq({link = VLink v;_},f_list,p_list) -> Neq(v,f_list,p_list)
  ...
  | Eq({link = VLink v;_},context) -> Eq(v,existentialise_context free_vars map context)
  | Eq(var,context) when List.exists (is_equal_variable var) free_vars -> Eq(var,existentialise_context free_vars map context)
  | Eq(var,context) ->
      let new_var = fresh_variable (get_support var) in
      link_var var new_var;
      map := (new_var,var) :: !map;
      Eq(new_var,existentialise_context free_vars map context)
      
let existentialise_general_formula free_vars g_form =
  let map = ref [] in
  let new_formula = List.map (existentialise_atomic_formula free_vars map) g_form.formula in
  cleanup_var ();
  { g_form with formula = new_formula; map_exists_to_free = !map }
